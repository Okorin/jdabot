...
public class CommandHandler  extends DiscordGuildListenerAdapter {
	...
	public MessageReceivedCommand massKick(boolean excluding) {
		return (event, args) -> {
			...
			TextChannel logChannel = this.getChannelFromDb(event, GUILD_SETTINGS, GUILD_SETTINGS.LOG_CHANNEL, GUILD_SETTINGS.ID);
	...
	private void kickOne(Guild guild, Member member, TextChannel logChannel) {
		if (member != null &&
		    !member.getUser().getId().equals(env.botId) &&
			...
			if (!args.isEmpty()) {
				if (args.get(0).length() > 3) {
					...
					if (member != null) {
						...
						event.getChannel().sendMessage("The Roles of " + member.getUser().getName() + "#" + member.getUser().getDiscriminator()+ ":" + rol).queue();
					} else {
	...
	private boolean rateLimit() {
		...
		if (commandCounter == 0) firstCommandInCounter = now;
		...
		if (commandCounter == rateLimitMax) cooldownUntil = now + rateLimitCooldown;
